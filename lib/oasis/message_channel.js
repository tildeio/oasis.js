  ...
  onRequest: function(eventName, callback, binding) {
    ...
    this.on('@request:' + eventName, function(data) {
      ...
      RSVP.resolve(getResponse).then(function (value) {
        ...
        // In the case of `Error`s, it may be useful to transfer the stack as
        // well.  The easiest way to do this would be to just send a simple
        // object with `message` and `stack` properties.
        var value = error instanceof Error ? error.message : error;
...
export { OasisPort, PostMessageMessageChannel, PostMessagePort };
